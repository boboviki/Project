# MD5加密：
# 全称：MD5消息摘要算法(英语：MD5 Message-Digest Algorithm)，
...
# SHA1加密:
# 全称：安全哈希算法(Secure Hash Algorithm)
...
# HMAC加密
# 全称：散列消息鉴别码(Hash Message Authentication Code)，
...
# ECC加密
# 全称：椭圆曲线加密（Elliptic Curve Cryptography），ECC加密算法是一种公钥加密技术，
# 以椭圆曲线理论为基础。利用有限域上椭圆曲线的点构成的Abel群离散对数难解性，实现加密、解密和数字签名。
...
# RSA加密
# 全称：Rivest-Shamir-Adleman，RSA加密算法是一种非对称加密算法。
...
if __name__ == "__main__":
    #if __name__ == "__main__":，文件作为脚本直接执行）才会被执行，而import到其他脚本中是不会被执行的。
    print("*************ECC椭圆曲线加密*************")
    # print("*************ECC椭圆曲线加密*************")
    # ecc_main()//RSA  //CRC  //仿射码

    RsaUtil.decrypt_by_private_key("0x03,0x05,0x14,0x45,0xDE,0x92")